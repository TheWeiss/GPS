...
def print_anti_measure(all_ASR, anti_index, need_log=False, path=None):
    ...
    low = np.round(anti_MIC['measurement'].min())
    high = np.round(anti_MIC['measurement'].max())
    ...
    pd.DataFrame(bins_count['measurement'].tolist(), index= bins_count.index, columns=hist_range[:-1]+0.5).T.plot.bar(stacked=True, figsize=(15,10))
    plt.title(anti)
